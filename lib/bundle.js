...
module.exports = async (
    ...
    localesPaths.forEach(async (localePath) => {
        ...
        bundlingConfig.forEach(async (bundle) => {
            ...
            // Get mixins map to exclude from config file
            const filePath = path.join(process.cwd(), 'mixins-map.json');
            const rawData = fs.readFileSync(filePath, 'utf8');
            const mixinsToSkip = JSON.parse(rawData);
            logger.debug(`Collected mixins map from ${filePath}`);
            ...
            for (const moduleName in bundle.modules) {
                ...
                try {
                    ...
                    //Do not push mixins to map so JS will work as expected after Magento fix
                    if (!mixinsToSkip[moduleName]) {
